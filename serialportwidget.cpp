...
void SerialPortWidget::on_portComboBox_currentIndexChanged(int index)
{
  if (index == -1) {
    ...
    if (!serialPortInfoList[index].hasVendorIdentifier())
      ui->vidLabel->setText(QLatin1String("-"));
    else
      ui->vidLabel->setText(QLatin1String("0x") + QString(QByteArray::number(serialPortInfoList[index].vendorIdentifier(), 16)));

    quint16 pid = serialPortInfoList[index].productIdentifier();

    if (!serialPortInfoList[index].hasProductIdentifier())
      ui->pidLabel->setText(QLatin1String("-"));
    else
      ui->pidLabel->setText(QLatin1String("0x") + QString(QByteArray::number(serialPortInfoList[index].productIdentifier(), 16)));
